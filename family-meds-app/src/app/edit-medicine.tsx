...
interface Medicine {
  id: string; name: string; quantity: number; dosagePerIntake: number;
  lowStockThreshold: number; unit: string; scheduleTimes: string[]; notes: string | null;
}
...
export default function EditMedicineScreen() {
  ...
  useEffect(() => {
    ...
    api.get<Medicine>(`/medicines/${id}`)
      .then((res) => {
        ...
        setDosage(String(m.dosagePerIntake));
        setUnit(m.unit);
        setScheduleTimes(m.scheduleTimes ?? []);
        setNotes(m.notes ?? '');
        setLowStock(String(m.lowStockThreshold ?? 5));
      })
      .catch(() => {
        if (Platform.OS === 'web') window.alert('Failed to load medicine');
        else Alert.alert(t('common.error'), 'Failed to load medicine');
        router.back();
      })
  ...
  const save = async () => {
    ...
    try {
      ...
      const msg = e?.response?.data?.message || 'Update failed. Try again.';
  ...
  return (
    <KeyboardAvoidingView style={f.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={f.scroll}>
        <Text style={[f.title, isRTL && f.right]}>{t('medicines.editMedicine')}</Text>
        ...
        <TouchableOpacity
          ...
          onPress={save}
          disabled={loading}
          accessibilityLabel="update-medicine-button"
        >
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={f.saveTxt}>{t('medicines.updateMedicine')}</Text>}
        </TouchableOpacity>
      </ScrollView>
